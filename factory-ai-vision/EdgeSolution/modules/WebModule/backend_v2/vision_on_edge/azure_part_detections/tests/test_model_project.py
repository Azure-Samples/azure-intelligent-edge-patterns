...
class ProjectModelTestCases(CustomVisionTestCase):
    ...
    def setUp(self):
        ...
        for i in range(3):
            Camera.objects.create(
                name="demo_camera_{i}", rtsp="0", area="{i*2},{i*3}", is_demo=True
            )
            Camera.objects.create(
                name="camera_{i}", rtsp="0", area="{i*2},{i*3}", is_demo=False
            )

            Location.objects.create(
                name=f"location_{i}", description=f"description_{i}", is_demo=True
            )
            Location.objects.create(
                name=f"demo_location_{i}", description=f"description_{i}", is_demo=False
            )
            Part.objects.create(
                name=f"part_{i}", description=f"description_{i}", is_demo=True
            )
            Part.objects.create(
                name=f"part_{i}", description=f"description_{i}", is_demo=False
            )
        self.exist_num = 3
    ...
    def test_setup_is_valid(self):
        ...
        self.assertEqual(Camera.objects.filter(is_demo=True).count(), self.exist_num)
        self.assertEqual(Camera.objects.filter(is_demo=True).count(), self.exist_num)
        self.assertEqual(Location.objects.filter(is_demo=True).count(), self.exist_num)
        self.assertEqual(Location.objects.filter(is_demo=True).count(), self.exist_num)
        self.assertEqual(Part.objects.filter(is_demo=True).count(), self.exist_num)
        self.assertEqual(Part.objects.filter(is_demo=True).count(), self.exist_num)
