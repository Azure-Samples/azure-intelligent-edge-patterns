...
def test_notification_detail(notification: Notification):
    ...
    assert (reverse("api:notification-detail",
                    kwargs={"pk": notification.id
                           }) == f"/api/notifications/{notification.id}")
    assert resolve(f"/api/notifications/{notification.id}"
                  ).view_name == "api:notification-detail"
...
def test_notification_delete_all():
    ...
    """
    assert reverse(
        "api:notification-delete-all") == "/api/notifications/delete_all"
    assert resolve("/api/notifications/delete_all"
                  ).view_name == "api:notification-delete-all"
