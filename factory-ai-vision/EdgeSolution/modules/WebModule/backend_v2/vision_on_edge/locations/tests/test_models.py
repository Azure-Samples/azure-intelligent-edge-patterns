...
class LocationTestCase(APITransactionTestCase):
    ...
    def setUp(self):
        ...
        Location.objects.create(
            name="Location1", description="description1", is_demo=False
        )

        Location.objects.create(
            name="Location2", description="description2", is_demo=False
        )

        Location.objects.create(
            name="DemoLocation1", description="SELECT * FROM 'LOCATION'", is_demo=True
        )

        Location.objects.create(
            name="DemoLocation2", description="python apps.py", is_demo=True
        )
        for special_string in special_strings:
            Location.objects.create(
                name=special_string, description=special_string, is_demo=False
            )
