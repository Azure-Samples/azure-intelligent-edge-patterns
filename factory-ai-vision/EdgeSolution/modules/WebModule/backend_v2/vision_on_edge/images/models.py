...
from .exceptions import (ImageLabelOutOfRange,
                         ImageGetRemoteImageRequestsError)
...
class Image(models.Model):
    ...
    def get_remote_image(self):
        ...
        try:
            ...
            raise ImageGetRemoteImageRequestsError(detail=("url: " +
                                                           self.remote_url))
    ...
    def set_labels(self, left: float, top: float, width: float, height: float):
        ...
        if left > 1 or top > 1 or width > 1 or height > 1:
            logger.error("%s, %s, %s, %s must be less than 1", left, top,
                         width, height)
            return
