...
class LocationRestTestCases(APITransactionTestCase):
    ...
    def setUp(self):
        ...
        data = {'name': 'DemoLocation1',
                'description': 'Desb1',
                'is_demo': True}
        self.client.post(url, data, format='json')

        data = {'name': 'DemoLocation2',
                'description': 'Desb1',
                'is_demo': True}
        self.client.post(url, data, format='json')

        data = {'name': 'DemoLocation3',
                'description': 'Desb1',
                'is_demo': True}
        self.client.post(url, data, format='json')
    ...
    def test_create_location_1(self):
        ...
        self.assertEqual(len(json.loads(response.content)),
                         self.exist_num+1)
        # Check using db
        self.assertEqual(Location.objects.count(),
                         self.exist_num+1)
        self.assertEqual(Location.objects.get(name=location_name).name,
                         location_name)
