...
from ..api.serializers import CameraTaskSerializer, PDScenarioSerializer
from ..api.views import CameraTaskViewSet
...
def test_get(part_detection):
    ...
    pd_list_view = CameraTaskViewSet.as_view({"get": "list"})
    request = factory.get("/fake-url/")

    response = pd_list_view(request).render()
    response_body = response.content.decode("utf-8")

    assert CameraTaskSerializer(part_detection).data in json.loads(response_body)
...
def test_put_from_frontend(part_detection):
    ...
    factory = APIRequestFactory()
    pd_list_view = CameraTaskViewSet.as_view({"put": "update"})
