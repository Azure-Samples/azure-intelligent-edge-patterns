...
- NOTE: The Android SDK that came installed with Android Studio should be located at `C:\Users\<yourUser>\AppData\Local\Android\Sdk` (If it is in a different location, replace the path in the export command with the correct location.)

2. Set JAVA_HOME with:

- Get the path to your Java jdk installed with: `which javac`
- If there is a `bin/javac` at the end of the path, remove it and copy the remaining path to the clipboard (Ex: `/c/Program Files/Java/jdk1.8.0_221`)

```
echo "export JAVA_HOME='<path-copied-from-above>'" >> ~/.bashrc
```

3. Now run the command: `source ~/.bashrc`
...
2. Navigate to the root folder of the mobile-bluetooth-app project.
3. Run the following script in a terminal using the values you assembled from the `Prerequisite Data Before Building` Section:
...
4. The script will produce an .apk file (`app-release.apk`) on your computer at the root project folder in `patient-bluetooth-connect-app/android/app/build/outputs/apk/release/app-release.apk` # TODO: update this path if necessary
...
3. Make sure you are in the Android app project folder (`patient-bluetooth-connect-app` #TODO: update this folder name if necessary).
4. Run `adb install ./android/app/build/outputs/apk/release/app-release.apk`.
...
TODO: this is a command to generate a debug key if needed for the develop section:
