...
class AnalyticsAPI:
    ...
    def score(self, pilImage):
        try:
                ...
                if self.initialized:
                    ...
                    for i in range(len(resDict)):
                        ...
                        try:
                            ...
                            xmax = resDict[i]["xmax"]
                            ymax = resDict[i]["ymax"]
                            state = self.cache.checkCache(resDict[i]["plate"], (xmax-xmin), (ymax-ymin)) #why do we get multiple of same plate given this logic?

                        except Exception as e:
                            self.logger.info("[AI EXT] Exception warning (ScoringAPI - Score): {0}".format(str(e)))
                            self.logger.info("[AI EXT] AHAHHAHAHHAHAHAHA DUMMY it don't work, why?")
                            state = "no_detection"
            ...
            #self.logger.info("[AI EXT] AHAHHAHAHHAHAHAHA DUMMY 3 it don't work, why? your container name is %s your blobfolder is %s and your filename is %s", self.containerName, self.blobfolder, self.filename)
            resJson = OrderedDict()
