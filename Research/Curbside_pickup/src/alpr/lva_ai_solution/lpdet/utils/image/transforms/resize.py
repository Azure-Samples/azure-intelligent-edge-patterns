...
import cv2
import numpy as np
from .geometry import impad_to_multiple
...
def img_scale_transform(img, scale, size_divisor=None, keep_ratio=True):
    h, w = img.shape[:2]
    if keep_ratio:
        max_long_edge = max(scale)
        max_short_edge = min(scale)
        ...
        scale_w = scale[0] / w
        scale_h = scale[1] / h
        scale_factor = np.array([scale_w, scale_h], dtype=np.float32)
    img_shape = img.shape
    if size_divisor is not None:
        img = impad_to_multiple(img, size_divisor)
        pad_shape = img.shape
    else:
        pad_shape = img_shape
