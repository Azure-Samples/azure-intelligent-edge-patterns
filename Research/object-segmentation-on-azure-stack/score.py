...
def run(data):
    try:
        ...
        start_at = time.time()
        img_data_list = json.loads(data)
        img_tensor_list = [torch.tensor(item) for item in img_data_list]
        model.eval()
        with torch.no_grad():
            predictions = model([item.to(device) for item in img_tensor_list])

        pred_data_list = [prediction['masks'][0, 0].mul(255).byte().cpu().numpy().tolist() for prediction in predictions]
        return {"predicts": pred_data_list,
